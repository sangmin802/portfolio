...
function App({closePop, selectProject, resetPageYOffset, selectProjectType, projectPop, MY_CODING, STACKS, data, props, Project, Stack, StackWrap, TabBtn}){  
  ...
  return (
    <div className={`App ${popupBg ? 'App100vh' : ''}`}>
      ...
      <div className="appBg" style={{display : popupBg ? 'block' : 'none'}}
        onClick={closePop}
      ></div>
      {projectPop}
...
function CreateUseCallbackMethods(isReady){
  const movingTab = useCallback((index) => {
    let top = 0;
    if(index!==0 && isReady){
